...
int main(int argc, char** argv) {
	...
	cout << "Michael Dutchin -- CSIS 215 Programming Assignment 1 -- Bag Dictionary" << endl << endl;
	...
	cout << "Testing bag with <int, string> KV Pair\n";
	ABag<KVpair<int, string>> myBag(5); //Used to test bag functions not previously demonstrated

	//make test values
	KVpair<int, string> test1(1, "A");
	KVpair<int, string> test2(2, "B");
	KVpair<int, string> test3(3, "C");
	KVpair<int, string> test4(4, "D");
	KVpair<int, string> test5(5, "E");

	//populate myBag with test values
	myBag.addItem(test1);
	myBag.addItem(test2);
	myBag.addItem(test3);
	myBag.addItem(test4);
	...
	cout << "INSERT: Size of myBag is " << myBag.size() << endl;

	//INSPECT TOP: myBag
	KVpair<int, string> inspect(0, " ");
	myBag.inspectTop(inspect);
	cout << "INSPECT TOP: The top of myBag has a key of " << inspect.key() << " and a value of " 
		 << inspect.value() <<endl;

	//BAG CAPACITY: myBag
	cout << "INSERT: Capacity of myBag is " << myBag.bagCapacity() << endl;
